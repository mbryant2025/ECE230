...
class MOSCAP(Silicon):

    def __init__(self, T = 300 * kelvin):
        super().__init__(T)
        self.calculation_functions.extend([self.calc_Na_and_Nd, self.calc_phi_PM, self.calc_phi_NM, self.calc_phi_FB, self.calc_V_SCB_at_V_TN,
                                           self.calc_Vdep_SCB, self.calc_W_dB_at_V_TN, self.calc_L_DB, self.calc_Q_SCB_at_V_TN,
                                           self.calc_Q_G_at_V_GB, self.calc_epsilon_xOX_at_V_TN, self.calc_V_ox_at_V_TN, self.calc_epsilon_xB,
        ...
        moscap_quantities = {
            ...
            'Q_SCB@V_TN' : None, #Substrate space charge density at the substrate space-charge region voltage
            'L_DB' : None, #Extrinsic Debye length
            'Q_G@V_GB' : None, #Gate charge density at the gate bulk voltage
            'epsilon_xOX@V_TN' : None, #Oxide electric field
            'V_ox@V_TN' : None, #Oxide potential difference
            'epsilon_xB@V_TN' : None, #Transverse electric-field distribution
    ...
    def calc_Na_and_Nd(self):
        # For consistency between the silicon and moscap classes
        if self.should_calculate(needed=['N_aB']):
            self.known_quantities['N_a'] = self.known_quantities['N_aB']
        if self.should_calculate(needed=['N_dB']):
            self.known_quantities['N_d'] = self.known_quantities['N_dB']
    ...
    def calc_Q_SCB_at_V_TN(self):
        ...
        Q_SCB_at_V_TN = -math.sqrt(2) * epsilon_Si_ * k_BT_300K_ / (q_ * L_DB) * sp.sqrt(inner1 + (no_B / po_B) * inner2) * C / cm**2
        self.known_quantities['Q_SCB@V_TN'] = Q_SCB_at_V_TN

    def calc_Q_G_at_V_GB(self):
        if not self.should_calculate(needed=['Q_f', 'Q_it']):
            return
        # NOTE assumes Q_SCB is zero here
        Q_f = self.known_quantities['Q_f']
        Q_it = self.known_quantities['Q_it']
        Q_G_at_V_GB = -(Q_f + Q_it)
        self.known_quantities['Q_G@V_GB'] = Q_G_at_V_GB

    def calc_epsilon_xOX_at_V_TN(self):
        if not self.should_calculate(needed=['Q_SCB@V_TN']):
            return
        Q_SCB_at_V_TN = self.known_quantities['Q_SCB@V_TN']
        epsilon_xOX_at_V_TN = -Q_SCB_at_V_TN / epsilon_ox * farad / coulomb * volt
        self.known_quantities['epsilon_xOX@V_TN'] = epsilon_xOX_at_V_TN

    def calc_V_ox_at_V_TN(self):
        if not self.should_calculate(needed=['epsilon_xOX@V_TN', 'X_ox']):
            return
        epsilon_xOX = self.known_quantities['epsilon_xOX@V_TN']
        X_ox = self.known_quantities['X_ox']
        V_ox = epsilon_xOX * X_ox
        self.known_quantities['V_ox@V_TN'] = V_ox

    def calc_epsilon_xB(self):
        if not self.should_calculate(needed=['Q_SCB@V_TN', 'X_ox']):
            return
        Q_SCB_at_V_SCB = remove_units(self.known_quantities['Q_SCB@V_TN'])
